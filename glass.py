...
class GLASS(torch.nn.Module):
    ...
    def trainer(self, training_data, val_data=None, name=None):
    ...
    def _train_discriminator(self, input_data, cur_epoch, pbar, pbar_str1):
        ...
        for i_iter, data_item in enumerate(input_data):
            ...
            img = data_item["image"]
            img = img.to(torch.float).to(self.device)
            print(f"image: {img.shape} aug: {aug.shape}")
            if self.pre_proj > 0:
            ...
            print(f"fake_feats: {fake_feats.shape} true_feats: {true_feats.shape}")

            mask_s_gt = data_item["mask_s"].reshape(-1, 1).to(self.device)
            print(f"mask_s: {data_item["mask_s"].shape} mask_s_gt: {mask_s_gt.shape}")
